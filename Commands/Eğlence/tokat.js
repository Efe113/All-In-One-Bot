  ...
  async execute(interaction) {
    ...
    if (hedef.id === tokatlayan.id) {
      return interaction.reply({
        content: '<a:dikkat_arviis:1421402093876674641> **Kendi kendini tokatlayamazsın...**',
        flags: 64
      });
    }
    ...
    if (!fs.existsSync(dosyaYolu)) {
      return interaction.reply({ content: '<a:dikkat_arviis:1421402093876674641> **GIF dosyası bulunamadı.**', flags: 64 });
    }
    ...
    try {
      await interaction.editReply({
        content: `🤬 <@${tokatlayan.id}> <:sadesagok_arviis:1421412348350038058> <@${hedef.id}> kişisine sağlam bir tokat attı!`,
        files: [dosyaYolu]
      });
    } catch (error) {
      console.error('Interaction hatası:', error);
      try {
        await interaction.editReply({ content: '<a:dikkat_arviis:1421402093876674641> **Mesaj gönderilirken hata oluştu.**' });
      } catch (innerError) {
