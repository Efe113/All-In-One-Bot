  ...
  async execute(interaction) {
    ...
    if (subcommand === 'oyna') {
      ...
      if (userChoice === botChoice) {
        result = '<a:tokustur_arviis:1421412961154629704> BERABERE! İkiniz de aynı seçimi yaptınız...';
      } else if (
        (userChoice === 'rock' && botChoice === 'scissors') ||
        (userChoice === 'paper' && botChoice === 'rock') ||
        (userChoice === 'scissors' && botChoice === 'paper')
      ) {
        result = '<a:mutlupanda_arviis:1421409835160834098> KAZANDIN! Tebrikler.';
        const wins = data[winsKey] || 0;
        data[winsKey] = wins + 1;
      } else {
        result = '<:uzgunpanda_arviis:1421413190092460102> KAYBETTİN! Daha şanslı olabilirsin.';
      }
      ...
      const TKMEmbed = new EmbedBuilder()
        ...
        .setTitle(result)
        .setDescription(`
<:girisok_arviis:1421408917816082515> Zorluk: ${difficulty === "hard" ? "**Zor ❤️**" : "**Kolay 💚**"}
...
-# <:modernsagok_arviis:1421409769788407899>Liderlik tablosunu görmek için \`/taş-kağıt-makas top\` komutunu kullanabilirsin.
      `);
      ...
      for (let i = 0; i < leaderboard.length; i++) {
        ...
        leaderboardText += `**\`${prefix}\`** ${username} <:sadesagok_arviis:1421412348350038058> **${leaderboard[i].wins} kez** kazanmış.\n`;
      }
