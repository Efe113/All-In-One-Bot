...
module.exports = {
  ...
  return interaction.reply({
    content: `<:tik_arviis:1421412906435739698> ${kaynak} kanalından ${hedef} kanalına yönlendirme **ayarlandı.**` +
      `${rol ? `\n<:ampul_arviis:1421401217040515142> Rol: <@&${rol.id}>` : ""}`,
    flags: 64
  });
}
    ...
    if (sub === "listele") {
      ...
      if (yönlendirmeler.length === 0) {
        return interaction.reply({
          content: "<:carpi_arviis:1421401682348216340> Hiç yönlendirme **ayarlanmamış.**",
          flags: 64
        });
      }

      const liste = yönlendirmeler.map((y, i) =>
        `**#${i + 1}**: <#${y.kaynakId}> <:sadesagok_arviis:1421412348350038058> <#${y.hedefId}>` +
        `${y.rolId ? ` **(** <:ampul_arviis:1421401217040515142> <@&${y.rolId}> **)**` : ""}`
      ).join("\n");

      return interaction.reply({
        content: `<:hashtag_arviis:1421409016701128794> **Yönlendirme Listesi:** \n\n${liste}`,
        flags: 64
      });
    }
    ...
    if (sub === "sil") {
      ...
      return interaction.reply({
        content: `<:tik_arviis:1421412906435739698> ${kaynak} kanalının yönlendirmesi **silindi.**`,
        flags: 64
      });
    }
  }
