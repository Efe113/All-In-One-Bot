...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (subcommand === 'ayarla') {
        ...
        if (customEmojiMatch) {
          ...
          if (!foundEmoji) {
            return interaction.reply({
              content: `<a:dikkat_arviis:1421402093876674641> Girilen emoji **(** ${emoji} **)** bu sunucuda veya botun bulunduğu diğer sunucularda bulunmuyor.`,
              flags: 64
            });
          }
        }
      ...
      return interaction.reply({
        content: `<:tik_arviis:1421412906435739698> <#${kanal.id}> kanalına şu emoji(ler) eklenecek: ${emojiler.join(' ')} \n-# <:sadesagok_arviis:1421412348350038058> Bot mesajları: **${includeBots ? '__Dahil__' : '__Hariç__'}**`,
        flags: 64
      });
    }
    ...
    if (subcommand === 'sıfırla') {
      if (!veri[key]) {
        return interaction.reply({
          content: `<a:dikkat_arviis:1421402093876674641> **<#${kanal.id}> kanalında ayarlanmış bir emoji sistemi yok.**`,
          flags: 64
        });
      }
      ...
      return interaction.reply({
        content: `<:tik_arviis:1421412906435739698> <#${kanal.id}> kanalındaki emoji sistemi başarıyla **sıfırlandı.**`,
        flags: 64
      });
    }
  }
