...
module.exports = {
  ...
  if (!bot.bot) {
    return interaction.reply({
      content: `<a:dikkat_arviis:1421402093876674641> **Bir bot seç.**`,
      flags: 64
    });
  }
  ...
  if (ayniKuralVar) {
    return interaction.reply({
      content: `<:carpi_arviis:1421401682348216340> Bu bot için bu kanalda zaten bir etiketleme kuralı **ayarlanmış.**`,
      flags: 64
    });
  }
  ...
  await interaction.reply(`<:tik_arviis:1421412906435739698> Yeni kural eklendi: ${kanal} kanalında ${bot} mesaj attığında <@&${rol.id}> **etiketlenecek.**`);
}

    if (subcommand === 'sıfırla') {
      saveData([]);
      await interaction.reply('<:tik_arviis:1421412906435739698> Tüm etiketleme kuralları **sıfırlandı.**');
    }
  }
};
